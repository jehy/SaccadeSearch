...
void CSaccadeSearchDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_CHECK9, IgnoreStumulCode02);
}
...
BEGIN_MESSAGE_MAP(CSaccadeSearchDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_CHECK9, &CSaccadeSearchDlg::OnBnClickedCheck9)
END_MESSAGE_MAP()
...
{
	for (unsigned int i = 0; i < StimulNegative.size(); i++)
		if (StimulNegative.at(i) == stimul)
			return -1;
	for (unsigned int i = 0; i < StimulPositive.size(); i++)
...
{
	//next saccade
	unsigned int z = Conan->Saccades.size();
	if (z == 0)
		return;
